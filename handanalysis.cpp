...
void HandAnalysis::detect(const cv::Mat & input_frame)
{
    ...
    auto contour = _contourExtract();
    if (!contour.empty())
    {
        _gestureRecognize(std::move(contour));
        emit detected();
    }
...
const HandAnalysis::Contour HandAnalysis::_contourExtract()
{
    filtered_frame.copyTo(convexity_frame);
    interested_frame.copyTo(contour_frame);
    std::vector<Contour > contours;
    cv::findContours(convexity_frame, contours, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_NONE);
    cv::cvtColor(convexity_frame, convexity_frame, cv::COLOR_GRAY2RGB);

    // Find out the biggest contour
    int current_area, biggest_area = 0;
    size_t total_contours = contours.size();
    size_t indx = total_contours;
    for (size_t i = -1; ++i < total_contours;)
    {
        current_area = cv::contourArea(contours[i]);
        if (current_area > DEFAULT_DETECTION_AREA && current_area > biggest_area)
        {
            biggest_area = std::move(current_area);
            indx = i;
        }
    }

    if (indx < total_contours)
    {
        // Draw contour
        cv::drawContours(convexity_frame, contours, indx, ColorRed);
        cv::drawContours(contour_frame, contours, indx, ColorRed);

        // Find and Draw Convex Hull
        std::vector<std::vector<cv::Point> > hulls(1);
        cv::convexHull(contours[indx], hulls[0], false);
        ...
        if (hulls[0].size()>0)
        {
            ...
            cv::RotatedRect rect = cv::minAreaRect(contours[indx]);
...
void HandAnalysis::_gestureRecognize(const HandAnalysis::Contour & contour)
{
    ...
    // Step 0, draw the current_center of defects
    ...
    emit handPos(center_x, center_y);
    emit fingersFound(num_of_fingers);
}
